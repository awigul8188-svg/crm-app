...
export default function ImportModal({ onClose, onDone, onStatsRefresh }) {
  ...
  const canImport = cleared && (mode === 'sheets' ? sheetsUrl.trim() !== '' : !!file)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 14, padding: '20px 24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              {[
                { label: 'Orders', value: result.orders },
                { label: 'Line Items', value: result.items },
                ...
                <div key={s.label} style={{ textAlign: 'center', background: '#fff', borderRadius: 10, padding: '12px 8px' }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: s.color || '#0f172a', fontFamily: '"Bricolage Grotesque", sans-serif' }}>{s.value}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: cleared ? 0 : 10 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: cleared ? '#065f46' : '#991b1b' }}>
          ...
          {confirmClear && !cleared && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8, padding: '10px 14px', background: '#fee2e2', borderRadius: 10 }}>
              ...
              <button onClick={() => setConfirmClear(false)} style={{ background: '#fff', border: '1px solid #fca5a5', borderRadius: 8, padding: '6px 12px', fontSize: 12, cursor: 'pointer', color: '#991b1b' }}>Cancel</button>
              <button onClick={handleClear} disabled={clearing} style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{
          ...
          opacity: cleared ? 1 : 0.55, transition: 'opacity 0.2s',
        }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {[{ val: 'file', label: '📂 Upload file' }, { val: 'sheets', label: '🔗 Google Sheets URL' }].map(opt => (
              <button key={opt.val} onClick={() => cleared && setMode(opt.val)} style={{
                padding: '8px 16px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: cleared ? 'pointer' : 'not-allowed',
                border: `1.5px solid ${mode === opt.val ? BRAND : '#e2e8f0'}`,
                background: mode === opt.val ? '#f0fffe' : '#f8fafc',
                color: mode === opt.val ? '#0f172a' : '#94a3b8',
              }}>{opt.label}</button>
            ))}
          </div>
          ...
          {mode === 'file' ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ fontSize: 11, color: '#94a3b8', lineHeight: 1.6 }}>
                The sheet must be shared as <strong style={{ color: '#64748b' }}>Anyone with the link → Viewer</strong>.
                The CRM will fetch the latest data directly — no download needed.
              </div>
            </div>
          )}
        </div>
          ...
          <button className="btn btn-primary" onClick={handleImport} disabled={!canImport || importing}
            ...
            {importing ? (
              <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {mode === 'sheets' ? 'Fetching & Importing…' : 'Importing…'}
              </span>
            ) : mode === 'sheets' ? 'Import from Google Sheets' : 'Import Data'}
          </button>
        </div>
      </div>
    </Modal>
