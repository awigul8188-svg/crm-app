...
function Pagination({ page, pages, onChange }) {
  ...
  return (
    <div style={{ display:'flex', gap:8, justifyContent:'center', padding:'12px 0', alignItems:'center' }}>
      <button onClick={() => onChange(page-1)} disabled={page===1} style={{ padding:'5px 12px', borderRadius:8, border:'1px solid rgba(255,255,255,0.09)', background:'var(--card)', color:page===1?'#cbd5e1':'#475569', cursor:page===1?'not-allowed':'pointer', fontSize:12, fontWeight:600, fontFamily:'"Plus Jakarta Sans",sans-serif' }}>← Prev</button>
      <span style={{ fontSize:12, color:'rgba(255,255,255,0.50)' }}>Page {page} of {pages}</span>
      <button onClick={() => onChange(page+1)} disabled={page===pages} style={{ padding:'5px 12px', borderRadius:8, border:'1px solid rgba(255,255,255,0.09)', background:'var(--card)', color:page===pages?'#cbd5e1':'#475569', cursor:page===pages?'not-allowed':'pointer', fontSize:12, fontWeight:600, fontFamily:'"Plus Jakarta Sans",sans-serif' }}>Next →</button>
    </div>
  )
}
...
function InquiryAssignModal({ inquiryId, onClose, onSaved }) {
  ...
  if (!data) return createPortal(<div style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center' }}><div style={{ background:'var(--card)', borderRadius:20, padding:40, color:'rgba(255,255,255,0.38)' }}>Loading...</div></div>, document.body)
  ...
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'var(--card)', borderRadius:20, boxShadow:'0 24px 80px rgba(0,0,0,0.25)', width:'100%', maxWidth:680, maxHeight:'90vh', display:'flex', flexDirection:'column', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
        <div style={{ padding:'18px 24px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexShrink:0 }}>
          <div>
          ...
          {parts.map(part => (
            <div key={part.requirement_id} style={{ background: part.is_delayed?'#fff5f5':'#f8fafc', borderRadius:12, padding:'14px 16px', border:`1px solid ${part.is_delayed?'#fecaca':'#f1f5f9'}`, marginBottom:10 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', marginBottom:4 }}>Assign To</div>
                ...
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', marginBottom:4 }}>Urgency</div>
                ...
                <div style={{ gridColumn:'1/-1' }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', marginBottom:4 }}>Notes for Purchaser</div>
        ...
        <div style={{ padding:'14px 24px', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', gap:10, flexShrink:0 }}>
          <button onClick={onClose} style={{ flex:1, padding:11, borderRadius:12, border:'1px solid rgba(255,255,255,0.09)', background:'var(--card)', color:'rgba(255,255,255,0.65)', fontWeight:600, fontSize:13, cursor:'pointer', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>Cancel</button>
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:10, marginBottom:12, flexWrap:'wrap', alignItems:'center' }}>
        ...
        {result && <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)', marginLeft:'auto' }}>{result.total} total</div>}
      </div>
      {loading ? <div style={{ textAlign:'center', padding:48, color:'rgba(255,255,255,0.38)' }}>Loading...</div> : filtered.length===0 ? (
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:48, textAlign:'center', color:'rgba(255,255,255,0.38)' }}>No parts found</div>
      ) : (
              ...
              <tbody>
                {filtered.map((p, i) => (
                  <tr key={p.requirement_id} style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', background:p.is_delayed?'#fff5f5':p.is_over_selling?'#fff7ed':i%2===0?'#fff':'#fafbfc' }}>
                    ...
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.65)' }}>{p.quantity||'—'}</td>
                    <td style={{ padding:'9px 12px', fontWeight:500, whiteSpace:'nowrap' }}>{p.customer_name}<div style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>{p.customer_company}</div></td>
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.50)', whiteSpace:'nowrap' }}>{p.ae_name||'—'}</td>
                    ...
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.38)', fontSize:11, whiteSpace:'nowrap' }}>{formatDateShort(p.inquiry_date)}</td>
                    <td style={{ padding:'9px 12px' }}><AssignCell part={p} purchasers={purchasers} onAssign={handleAssign} /></td>
                    <td style={{ padding:'9px 12px' }}>
                      <span style={{ fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:12, ...(p.not_in_stock?{ background:'#fef2f2', color:'#dc2626' }:p.assignment_id?p.assignment_status==='quoted'?{ background:'#f0fdf4', color:'#16a34a' }:{ background:'#fff7ed', color:'#d97706' }:{ background:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.50)' }) }}>
                        {p.not_in_stock?'Not In Stock':p.assignment_id?p.assignment_status==='quoted'?'Quoted':'Pending':'Unassigned'}
                      </span>
                    </td>
                    <td style={{ padding:'9px 12px', maxWidth:140, color:'rgba(255,255,255,0.50)', fontSize:11, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.pm_notes||'—'}</td>
                    <td style={{ padding:'9px 12px', maxWidth:160 }}>
                      {p.quote_id ? (
                        <div>
                          ...
                          <div style={{ fontSize:11, color:'rgba(255,255,255,0.50)' }}>{p.condition}{p.lead_time?` · ${p.lead_time}`:''}</div>
                          <div style={{ fontSize:10, color:'rgba(255,255,255,0.38)' }}>{p.supplier_name}</div>
                        </div>
                      ) : <span style={{ color:'rgba(255,255,255,0.25)', fontSize:11 }}>No quote</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab==='dashboard' && ms && (
        <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:20 }}>
            <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:'16px 20px' }}>
              <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>Total Quoted Value</div>
              ...
              {!ms.byPurchaser?.length ? <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', padding:24, fontSize:13 }}>No purchasers assigned yet</div> : ms.byPurchaser.map(p => (
                <div key={p.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <div style={{ textAlign:'right', flexShrink:0 }}>
                    ...
                    <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>{p.pending_count} pending · {p.assigned} total</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:20 }}>
            ...
            {!ms.recentQuotes?.length ? <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', padding:24 }}>No quotes yet</div> :
              ms.recentQuotes.map((q, i) => {
                ...
                return (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.04)', background:isOver?'#fff5f5':'' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>{q.purchaser_name} · {timeAgo(q.updated_at)}</div>
                    </div>
                    <div style={{ textAlign:'right', flexShrink:0 }}>
                      <div style={{ fontWeight:700, color:isOver?'#dc2626':'#10b981', fontSize:13 }}>${q.price}</div>
                      <div style={{ fontSize:11, color:'rgba(255,255,255,0.50)' }}>{q.condition}</div>
                    </div>
                  </div>
                )
              })}
      ...
      {activeTab==='notifications' && (
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:14, alignItems:'center' }}>
            <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)' }}>Click to open inquiry and assign purchasers</div>
          ...
          {notifications.length===0 ? <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:60, textAlign:'center', color:'rgba(255,255,255,0.38)' }}><div style={{ fontSize:36, marginBottom:10 }}>📭</div>No new part notifications</div> : (
                ...
                return (
                  <div key={notif.id} onClick={() => { api.markNotificationRead(notif.id).catch(()=>{}); setNotifications(prev=>prev.map(n=>n.id===notif.id?{...n,read:1}:n)); if(notif.inquiry_id) setAssignInquiryId(notif.inquiry_id) }}
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      {notif.comment && <div style={{ fontSize:12, color:'rgba(255,255,255,0.50)', fontFamily:'monospace', background:'rgba(255,255,255,0.04)', padding:'3px 8px', borderRadius:6, marginBottom:3 }}>{notif.comment}</div>}
                      <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>{timeAgo(notif.created_at)} · Click to assign →</div>
                    </div>
        ...
        <Modal title="Add New Purchaser" onClose={() => { setShowNewUser(false); setNewUserForm({ name:'', username:'', password:'' }); setUserError('') }}>
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            {[['Full Name','name','text'],['Username','username','text'],['Password','password','password']].map(([l,k,t]) => (
              <div key={k}><div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>{l}</div>
            ...
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={() => setShowNewUser(false)} style={{ flex:1, padding:11, borderRadius:12, border:'1px solid rgba(255,255,255,0.09)', background:'var(--card)', color:'rgba(255,255,255,0.65)', fontWeight:600, fontSize:13, cursor:'pointer', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>Cancel</button>
        ...
        <Modal title="🔑 Purchaser Passwords Reset" onClose={() => setResetResults(null)} wide>
          ...
          <div style={{ border:'1px solid rgba(255,255,255,0.09)', borderRadius:12, overflow:'hidden', marginBottom:14 }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead><tr style={{ background:'rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>{['Name','Username','Password',''].map(h => <th key={h} style={{ textAlign:'left', padding:'8px 14px', fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)' }}>{h}</th>)}</tr></thead>
...
function QuotesTable({ dateRange }) {
  ...
  if (loading) return <div style={{ textAlign:'center', padding:48, color:'rgba(255,255,255,0.38)' }}>Loading...</div>
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:10, marginBottom:12 }}>
        ...
        {result && <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)', marginLeft:'auto' }}>{result.total} total quotes</div>}
      </div>
      {quotes.length===0 ? <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:48, textAlign:'center', color:'rgba(255,255,255,0.38)' }}>No quotes yet</div> : (
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', overflow:'hidden' }}>
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              <thead><tr style={{ background:'rgba(255,255,255,0.04)', borderBottom:'2px solid rgba(255,255,255,0.09)' }}>
              ...
              <tbody>
                {quotes.map((q, i) => (
                  <tr key={q.id} style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', background:q.is_over_selling?'#fff5f5':i%2===0?'#fff':'#fafbfc' }}>
                    ...
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.50)' }}>{q.ae_name||'—'}</td>
                    ...
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.50)' }}>{q.selling_price?`$${q.selling_price}`:'—'}</td>
                    <td style={{ padding:'9px 12px' }}>
                    ...
                    <td style={{ padding:'9px 12px' }}><span style={{ fontSize:11, background:'rgba(255,255,255,0.04)', padding:'2px 7px', borderRadius:5 }}>{q.condition}</span></td>
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.50)', fontSize:11 }}>{q.lead_time||'—'}</td>
                    <td style={{ padding:'9px 12px', color:'rgba(255,255,255,0.50)', fontSize:11 }}>{q.supplier_name||'—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
