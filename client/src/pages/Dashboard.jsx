              ...
              <thead>
                <tr style={{ background:'rgba(255,255,255,0.04)', position:'sticky', top:0 }}>
                  {['Date','Customer','Company','Assigned To','Disposition','Part Numbers'].map(h => (
                    <th key={h} style={{ textAlign:'left', padding:'10px 16px', fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.06em', whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} onClick={() => handleRowClick(r.id)} style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', cursor:'pointer', background: i%2===0 ? 'var(--card)':'var(--card-2)', transition:'background 0.1s' }}
                    ...
                    <td style={{ padding:'10px 16px', fontFamily:'monospace', fontSize:11, color:'rgba(255,255,255,0.65)', maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
...
function TopList({ data, label, color, showCompany, onDrilldown }) {
  ...
  return (
    <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid rgba(255,255,255,0.07)', padding:20 }}>
      ...
      {rows.length === 0 ? (
        <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', fontSize:13, padding:'24px 0' }}>No data for this period</div>
      ) : (
            ...
            <div key={row.name+i} style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ height:4, background:'rgba(255,255,255,0.04)', borderRadius:4 }}>
                  <div style={{ height:'100%', borderRadius:4, background:i===0?color:'rgba(255,255,255,0.25)', width:`${Math.round(row.count/max*100)}%`, transition:'width 0.3s' }} />
                </div>
              </div>
            </div>
          ))}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12, flexWrap:'wrap' }}>
        ...
        {preset === 'custom' && (
          <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--card)', border:`1px solid ${BRAND}40`, borderRadius:12, padding:'6px 14px' }}>
            <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} style={{ padding:'4px 8px', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, fontSize:13, outline:'none' }} />
            <span style={{ color:'rgba(255,255,255,0.38)' }}>→</span>
            <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} style={{ padding:'4px 8px', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, fontSize:13, outline:'none' }} />
          </div>
        )}
      </div>
      {/* Filter dropdowns */}
...
function LeadsTab({ filters, onDrilldown }) {
  ...
  return (
    <div>
      <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:12 }}>Today</div>
      ...
      <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:12 }}>Period Summary</div>
      ...
      <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid rgba(255,255,255,0.07)', padding:20, marginBottom:20 }}>
      ...
      <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid rgba(255,255,255,0.07)', padding:20 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
          ...
          <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>Click a row to drill down</div>
        </div>
            ...
            <thead>
              <tr style={{ borderBottom:'2px solid #f1f5f9' }}>
                {['Rep','Total','Today','Month','Won','Lost','Quoted','Bidding','Fake','No Resp.','Cold','Win Rate','Top Source'].map(h => (
                  <th key={h} style={{ textAlign:h==='Rep'?'left':'center', padding:'8px 10px', fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.06em', whiteSpace:'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(data.aePerformance||[]).filter(ae=>ae.name).map((ae,i) => {
                ...
                return (
                  <tr key={ae.id||ae.name} onClick={() => drill(`${ae.name}'s Leads`, { assigned_to: String(ae.id) })} style={{ background:i%2===0?'var(--card)':'var(--row-alt)', borderBottom:'1px solid rgba(255,255,255,0.06)', cursor:'pointer', transition:'background 0.1s' }}
                    ...
                    <td style={{ padding:'12px 10px', textAlign:'center', color:'rgba(255,255,255,0.38)' }}>{ae.fake}</td>
                    <td style={{ padding:'12px 10px', textAlign:'center', color:'rgba(255,255,255,0.50)' }}>{ae.no_response}</td>
                    <td style={{ padding:'12px 10px', textAlign:'center', color:'rgba(255,255,255,0.50)' }}>{ae.cold}</td>
                    ...
                    <td style={{ padding:'12px 10px', textAlign:'center', fontSize:11, color:'rgba(255,255,255,0.50)', whiteSpace:'nowrap' }}>{topSrc}</td>
                  </tr>
                )
              })}
            </tbody>
...
function OrdersTab({ filters, onDrilldown }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:12 }}>Period</div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:12, marginBottom:24 }}>
      ...
      <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid rgba(255,255,255,0.07)', padding:20 }}>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
          <thead><tr style={{ borderBottom:'2px solid #f1f5f9' }}>{['Name','Total','Processed','Rate'].map(h => <th key={h} style={{ textAlign:h==='Name'?'left':'right', padding:'8px 10px', fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.06em' }}>{h}</th>)}</tr></thead>
          <tbody>
