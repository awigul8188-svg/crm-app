...
export default function InquiryList({ type, title }) {
  ...
  const hasFilters = filterDispositions.length || filterSources.length || search
  const clearFilters = () => { setFilterDispositions([]); setFilterSources([]); setSearch('') }
  ...
  return (
    <div className="p-8 fade-in">
      ...
      <div className="flex gap-2.5 mb-5 flex-wrap items-center">
        ...
        {type !== 'online_order' && (
          <MultiSelect placeholder="All Sources" options={LEAD_SOURCES} selected={filterSources} onChange={setFilterSources} />
        )}
        {hasFilters ? (
          <button onClick={clearFilters} className="btn btn-ghost btn-sm text-red-500 hover:bg-red-50">✕ Clear</button>
        ) : null}
      </div>
                      ...
                      <td className="table-cell" onClick={e => e.stopPropagation()}>
                        {user.role === 'manager' && (
                          <button onClick={e => handleDelete(e, inq.id)} disabled={deleting === inq.id}
                            className="btn-icon btn-sm text-red-400 hover:text-red-600 hover:bg-red-50 opacity-0 group-hover:opacity-100">
                            {deleting === inq.id ? '...' : '🗑'}
                          </button>
                        )}
                      </td>
