...
function Tip({ active, payload, label }) {
  ...
  return (
    <div style={{ background:'var(--card-2)', border:'1px solid var(--border)', borderRadius:10, padding:'8px 12px', fontSize:12 }}>
...
function MetricCard({ label, value, sub, color = BRAND, prefix = '', suffix = '', onClick }) {
  return (
    <div onClick={onClick} style={{ background:'var(--card)', borderRadius:14, border:`1px solid ${onClick ? 'var(--card-2)' : 'var(--card-2)'}`, padding:'14px 18px', position:'relative', overflow:'hidden', cursor:onClick?'pointer':'default', transition:'all 0.15s' }}
      ...
      {onClick && <div style={{ position:'absolute', top:8, right:10, fontSize:9, color:'var(--text-4)', fontWeight:600 }}>VIEW ↗</div>}
      <div style={{ fontSize:10, fontWeight:700, color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>{label}</div>
      <div style={{ fontSize:26, fontWeight:800, color:'var(--text)', fontFamily:'"Bricolage Grotesque",sans-serif', lineHeight:1 }}>{prefix}{typeof value === 'number' ? value.toLocaleString() : (value ?? '—')}{suffix}</div>
...
function STitle({ children, right }) {
  return <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
    <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:700, fontSize:14, color:'var(--text)' }}>{children}</div>
    {right}
  </div>
}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
        ...
        {preset === 'custom' && (
          <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--card)', border:`1px solid ${BRAND}40`, borderRadius:12, padding:'6px 14px' }}>
            <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} style={{ padding:'4px 8px', border:'1px solid var(--border)', borderRadius:8, fontSize:12, outline:'none' }} />
            <span style={{ color:'var(--text-3)' }}>→</span>
            <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} style={{ padding:'4px 8px', border:'1px solid var(--border)', borderRadius:8, fontSize:12, outline:'none' }} />
          </div>
        )}
      </div>
    </div>
  ...
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:999999, background:'rgba(0,0,0,0.6)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--card)', borderRadius:20, boxShadow:'0 32px 100px rgba(0,0,0,0.3)', width:'100%', maxWidth:660, maxHeight:'90vh', overflowY:'auto', animation:'modalIn 0.18s ease-out', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
        ...
        {/* Header */}
        <div style={{ padding:'18px 24px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, background:'var(--modal-bg)', zIndex:10, borderRadius:'20px 20px 0 0' }}>
          {loading ? <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:700, fontSize:15, color:'var(--text)' }}>Loading...</div> : (
            <div>
              ...
              <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:700, fontSize:16, color:'var(--text)' }}>{inquiry?.customer_name}</div>
              {inquiry?.customer_company && <div style={{ fontSize:12, color:'var(--text-3)' }}>{inquiry?.customer_company}</div>}
            </div>
          )}
          <button onClick={onClose} style={{ width:32, height:32, borderRadius:10, border:'none', background:'var(--row-alt)', cursor:'pointer', fontSize:18, color:'var(--text-2)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>×</button>
        </div>
        ...
        {loading ? <Loader /> : (
          <div style={{ padding:'20px 24px 24px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Date</div>
                <input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} style={inp} />
              </div>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>{inquiry?.type === 'online_order' ? 'Status' : 'Disposition'}</div>
            ...
            {inquiry?.type === 'repeat' && (
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>PPC or Outbound</div>
            ...
            {inquiry?.type === 'online_order' && (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
                <div>
                  <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Verification</div>
                ...
                <div>
                  <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Order Amount</div>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase' }}>Part Numbers</div>
            ...
            {/* Notes */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Comments</div>
            ...
            <div style={{ borderTop:'1px solid var(--border)', paddingTop:16, marginBottom:16 }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:8 }}>Add Comment</div>
            ...
            {inquiry?.activity?.filter(a => a.action === 'Comment').length > 0 && (
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:8 }}>Comments</div>
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {inquiry.activity.filter(a => a.action==='Comment').map(a => (
                    <div key={a.id} style={{ background:'var(--row-alt)', borderRadius:10, padding:'10px 14px', border:'1px solid var(--border)' }}>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
                        <span style={{ fontSize:12, fontWeight:600, color:'var(--text)' }}>{a.user_name}</span>
                        <span style={{ fontSize:11, color:'var(--text-3)' }}>{timeAgo(a.created_at)}</span>
                      </div>
                      <div style={{ fontSize:13, color:'var(--text-2)' }}>{a.comment}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ borderTop:'1px solid var(--border)', paddingTop:16 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase' }}>Follow-ups</div>
              ...
              {showFollowupForm && (
                <div style={{ background:'var(--row-alt)', borderRadius:12, padding:14, border:'1px solid var(--border)', marginBottom:10 }}>
              ...
              {inquiry?.followups?.filter(f => !f.completed).map(fu => (
                <div key={fu.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid var(--border)' }}>
                  <div style={{ width:8, height:8, borderRadius:'50%', background: new Date(fu.follow_up_date) < new Date() ? '#ef4444' : BRAND, flexShrink:0 }} />
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:500, color:'var(--text-2)' }}>{fu.note}</div>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--card)', borderRadius:20, boxShadow:'0 24px 80px rgba(0,0,0,0.25)', width:'100%', maxWidth:900, maxHeight:'87vh', display:'flex', flexDirection:'column', animation:'modalIn 0.18s ease-out', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
        ...
        <div style={{ padding:'18px 24px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:'var(--text-3)', marginTop:2 }}>{loading ? 'Loading...' : `${rows.length} records — click any row to edit`}</div>
          </div>
        ...
        <div style={{ overflowY:'auto', flex:1 }}>
          {loading ? <Loader /> : rows.length === 0 ? (
            <div style={{ textAlign:'center', padding:60, color:'var(--text-3)' }}>No records found</div>
          ) : (
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead>
                <tr style={{ background:'var(--row-alt)', position:'sticky', top:0 }}>
                  {['Date','Customer','Company','Disposition','Parts','Notes',''].map(h => (
                    <th key={h} style={{ textAlign:'left', padding:'10px 14px', fontSize:11, fontWeight:700, color:'var(--text-2)', textTransform:'uppercase', letterSpacing:'0.06em', whiteSpace:'nowrap', borderBottom:'2px solid var(--border-2)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} style={{ borderBottom:'1px solid var(--border)', background:i%2===0?'var(--card)':'var(--row-alt)', transition:'background 0.1s' }}
                    ...
                    <td style={{ padding:'10px 14px', color:'var(--text-2)', fontFamily:'monospace', fontSize:12, whiteSpace:'nowrap' }}>{formatDateShort(r.created_at)}</td>
                    <td style={{ padding:'10px 14px', fontWeight:600, color:'var(--text)', whiteSpace:'nowrap' }}>{r.customer_name}</td>
                    <td style={{ padding:'10px 14px', color:'var(--text-2)', fontSize:12 }}>{r.customer_company||'—'}</td>
                    <td style={{ padding:'10px 14px' }}><DispositionBadge disposition={r.disposition} /></td>
                    <td style={{ padding:'10px 14px', fontFamily:'monospace', fontSize:11, color:'var(--text-2)', maxWidth:140, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r.requirements?.map(req => req.part_number).join(', ')||'—'}</td>
                    <td style={{ padding:'10px 14px', color:'var(--text-3)', fontSize:12, maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r.notes||'—'}</td>
...
function AELeadsTab({ dateFilters, onDrilldown }) {
  ...
  return (
    <div>
      <div style={{ fontSize:11, fontWeight:700, color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:10 }}>Today</div>
      ...
      <div style={{ fontSize:11, fontWeight:700, color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:10 }}>Period</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20, marginBottom:20 }}>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', padding:18 }}>
          ...
          {trendData.length === 0 ? <div style={{ height:160, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-3)' }}>No data</div> : (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {(data.bySource||[]).filter(s=>s.source).slice(0,7).map((s,i) => (
              <div key={s.source} onClick={() => drill(`Source: ${s.source}`)} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:2 }}><span style={{ color:'var(--text-2)' }}>{s.source}</span><span style={{ fontWeight:700 }}>{s.count}</span></div>
                <div style={{ height:4, background:'var(--row-alt)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:C[i%C.length], width:`${p.total>0?Math.round(s.count/p.total*100):0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
          ...
          {(data.byDisposition||[]).filter(d=>d.disposition).map((d,i) => {
            ...
            return (
              <div key={d.disposition} onClick={() => drill(d.disposition, { disposition:d.disposition })}
                style={{ background:'var(--row-alt)', borderRadius:12, padding:'12px 14px', border:'1px solid var(--border)', cursor:'pointer', transition:'all 0.15s' }}
                ...
                <div style={{ fontSize:11, color:'var(--text-2)', marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.disposition}</div>
                <div style={{ fontSize:20, fontWeight:800, color:'var(--text)', fontFamily:'"Bricolage Grotesque",sans-serif' }}>{d.count}</div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {(data.byDisposition||[]).slice(0,10).map((d,i) => (
              <div key={d.disposition} onClick={() => drill(d.disposition, { disposition:d.disposition })} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:2 }}><span style={{ color:'var(--text-2)' }}>{d.disposition||'Unknown'}</span><span style={{ fontWeight:700 }}>{d.count}</span></div>
                <div style={{ height:4, background:'var(--row-alt)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:C[i%C.length], width:`${p.total>0?Math.round(d.count/p.total*100):0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', padding:18 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {(data.bySource||[]).filter(s=>s.source).map((s,i) => (
              <div key={s.source} onClick={() => drill(`Source: ${s.source}`)} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:3 }}><span style={{ color:'var(--text-2)' }}>{s.source}</span><span style={{ fontWeight:700 }}>{s.count}</span></div>
                <div style={{ height:5, background:'var(--row-alt)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:C[i], width:`${p.total>0?Math.round(s.count/p.total*100):0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', padding:18 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
            {[['This Month',data.month.won,data.month.total,data.month.win_rate,'#10b981'],['This Year',data.year.won,data.year.total,data.year.win_rate,BRAND],['All Time',data.all.won,data.all.total,data.all.win_rate,'#6366f1']].map(([lbl,won,total,rate,clr]) => (
              <div key={lbl} style={{ background:'var(--row-alt)', borderRadius:12, padding:'12px 14px', textAlign:'center' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>{lbl}</div>
                <div style={{ fontSize:20, fontWeight:800, color:clr, fontFamily:'"Bricolage Grotesque",sans-serif' }}>{won}</div>
                <div style={{ fontSize:11, color:'var(--text-3)', marginTop:2 }}>won of {total}</div>
        ...
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', padding:18 }}>
          ...
          {!data.pipeline?.length ? <div style={{ textAlign:'center', color:'var(--text-3)', padding:'32px 0' }}>No active pipeline</div> : (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', padding:18 }}>
          <STitle>📅 My Follow-ups <span style={{ fontSize:12, color:'var(--text-3)', fontWeight:400 }}>{data.followups?.overdue?.length + data.followups?.today?.length > 0 ? `⚠ ${data.followups.overdue.length + data.followups.today.length} urgent` : ''}</span></STitle>
          {[...( data.followups?.overdue||[]).map(f=>({...f,urgency:'overdue'})), ...(data.followups?.today||[]).map(f=>({...f,urgency:'today'})), ...(data.followups?.upcoming||[]).map(f=>({...f,urgency:'upcoming'}))].slice(0,6).map(fu => (
            <div key={fu.id} onClick={() => onNavigate('inquiry-detail',{id:fu.inquiry_id})} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid var(--border)', cursor:'pointer' }}>
              ...
              <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:600, color:'var(--text)' }}>{fu.customer_name}</div><div style={{ fontSize:11, color:'var(--text-2)' }}>{fu.note}</div></div>
          ...
          {!data.followups?.overdue?.length && !data.followups?.today?.length && !data.followups?.upcoming?.length && <div style={{ textAlign:'center', color:'var(--text-3)', padding:'20px 0', fontSize:13 }}>✅ All caught up!</div>}
        </div>

        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid var(--border)', padding:18 }}>
          <STitle>🔔 Needs Attention <span style={{ fontSize:12, color:'var(--text-3)', fontWeight:400 }}>No activity 7+ days</span></STitle>
          {!data.untouched?.length ? <div style={{ textAlign:'center', color:'var(--text-3)', padding:'20px 0', fontSize:13 }}>✅ All up to date!</div> : data.untouched.map(inq => (
            <div key={inq.id} onClick={() => onDrilldown({ title:'Needs Attention', type:inq.type, filters:{ disposition: inq.disposition } })}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontWeight:600, fontSize:13, color:'var(--text)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{inq.customer_name}</div>
                <DispositionBadge disposition={inq.disposition} />
              </div>
              <div style={{ fontSize:11, color:'var(--text-3)', flexShrink:0 }}>{inq.last_activity ? timeAgo(inq.last_activity) : timeAgo(inq.created_at)}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ padding:32, maxWidth:1200, fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
        <div>
          <h1 style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:800, fontSize:26, color:'var(--text)', margin:0 }}>{greeting()}, {user.name} 👋</h1>
          <p style={{ color:'var(--text-3)', fontSize:14, marginTop:4 }}>{new Date().toLocaleDateString('en-US', { weekday:'long', month:'long', day:'numeric' })}</p>
        </div>
        <div style={{ display:'flex', gap:8 }}>
      ...
      {/* Tabs */}
      <div style={{ display:'flex', gap:2, background:'var(--row-alt)', borderRadius:14, padding:4, marginBottom:24, width:'fit-content' }}>
        {tabs.map(tab => (
