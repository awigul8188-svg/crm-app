...
function MetricCard({ label, value, sub, color = BRAND, prefix = '', suffix = '', onClick }) {
  return (
    <div onClick={onClick} style={{ background:'var(--card)', borderRadius:14, border:`1px solid ${onClick ? 'var(--card-2)' : 'var(--card-2)'}`, padding:'14px 18px', position:'relative', overflow:'hidden', cursor:onClick?'pointer':'default', transition:'all 0.15s' }}
      ...
      onMouseLeave={e => { e.currentTarget.style.borderColor='var(--card-2)'; e.currentTarget.style.boxShadow='none'; e.currentTarget.style.transform='none' }}>
      <div style={{ position:'absolute', top:0, left:0, width:3, height:'100%', background:color, borderRadius:'14px 0 0 14px' }} />
      {onClick && <div style={{ position:'absolute', top:8, right:10, fontSize:9, color:'rgba(255,255,255,0.25)', fontWeight:600 }}>VIEW ↗</div>}
      ...
      <div style={{ fontSize:26, fontWeight:800, color:'var(--card)', fontFamily:'"Bricolage Grotesque",sans-serif', lineHeight:1 }}>{prefix}{typeof value === 'number' ? value.toLocaleString() : (value ?? '—')}{suffix}</div>
...
function STitle({ children, right }) {
  return <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
    <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:700, fontSize:14, color:'var(--card)' }}>{children}</div>
    {right}
  </div>
}
  ...
  return (
    <div style={{ marginBottom:20 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
        ...
        {preset === 'custom' && (
          <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--card)', border:`1px solid ${BRAND}40`, borderRadius:12, padding:'6px 14px' }}>
            <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} style={{ padding:'4px 8px', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, fontSize:12, outline:'none' }} />
            <span style={{ color:'rgba(255,255,255,0.38)' }}>→</span>
            <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} style={{ padding:'4px 8px', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, fontSize:12, outline:'none' }} />
          </div>
        )}
      </div>
    </div>
...
function InquiryQuickEditModal({ id, onClose, onSaved }) {
  ...
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:999999, background:'rgba(0,0,0,0.6)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--card)', borderRadius:20, boxShadow:'0 32px 100px rgba(0,0,0,0.3)', width:'100%', maxWidth:660, maxHeight:'90vh', overflowY:'auto', animation:'modalIn 0.18s ease-out', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
        <style>{`@keyframes modalIn{from{opacity:0;transform:scale(0.96) translateY(8px)}to{opacity:1;transform:scale(1) translateY(0)}}`}</style>

        {/* Header */}
        <div style={{ padding:'18px 24px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, background:'var(--modal-bg)', zIndex:10, borderRadius:'20px 20px 0 0' }}>
          {loading ? <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:700, fontSize:15, color:'var(--card)' }}>Loading...</div> : (
            <div>
              ...
              <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:700, fontSize:16, color:'var(--card)' }}>{inquiry?.customer_name}</div>
              {inquiry?.customer_company && <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)' }}>{inquiry?.customer_company}</div>}
            </div>
          )}
          <button onClick={onClose} style={{ width:32, height:32, borderRadius:10, border:'none', background:'rgba(255,255,255,0.04)', cursor:'pointer', fontSize:18, color:'rgba(255,255,255,0.50)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>×</button>
        </div>
        ...
        {loading ? <Loader /> : (
          <div style={{ padding:'20px 24px 24px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Date</div>
                <input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} style={inp} />
              </div>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>{inquiry?.type === 'online_order' ? 'Status' : 'Disposition'}</div>
            ...
            {inquiry?.type === 'repeat' && (
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>PPC or Outbound</div>
            ...
            {inquiry?.type === 'online_order' && (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
                <div>
                  <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Verification</div>
                ...
                <div>
                  <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Order Amount</div>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase' }}>Part Numbers</div>
            ...
            {/* Notes */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:6 }}>Comments</div>
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:16, marginBottom:16 }}>
              <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:8 }}>Add Comment</div>
            ...
            {inquiry?.activity?.filter(a => a.action === 'Comment').length > 0 && (
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:8 }}>Comments</div>
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {inquiry.activity.filter(a => a.action==='Comment').map(a => (
                    <div key={a.id} style={{ background:'rgba(255,255,255,0.04)', borderRadius:10, padding:'10px 14px', border:'1px solid rgba(255,255,255,0.07)' }}>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
                        <span style={{ fontSize:12, fontWeight:600, color:'var(--card)' }}>{a.user_name}</span>
                        <span style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>{timeAgo(a.created_at)}</span>
                      </div>
                      <div style={{ fontSize:13, color:'rgba(255,255,255,0.65)' }}>{a.comment}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:16 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', letterSpacing:'0.08em', textTransform:'uppercase' }}>Follow-ups</div>
              ...
              {showFollowupForm && (
                <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:12, padding:14, border:'1px solid rgba(255,255,255,0.09)', marginBottom:10 }}>
              ...
              {inquiry?.followups?.filter(f => !f.completed).map(fu => (
                <div key={fu.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ width:8, height:8, borderRadius:'50%', background: new Date(fu.follow_up_date) < new Date() ? '#ef4444' : BRAND, flexShrink:0 }} />
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:500, color:'rgba(255,255,255,0.65)' }}>{fu.note}</div>
...
function DrilldownModal({ title, type, filters, onClose }) {
  ...
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--card)', borderRadius:20, boxShadow:'0 24px 80px rgba(0,0,0,0.25)', width:'100%', maxWidth:900, maxHeight:'87vh', display:'flex', flexDirection:'column', animation:'modalIn 0.18s ease-out', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
        <style>{`@keyframes modalIn{from{opacity:0;transform:scale(0.96) translateY(8px)}to{opacity:1;transform:scale(1) translateY(0)}}`}</style>
        ...
        <div style={{ padding:'18px 24px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)', marginTop:2 }}>{loading ? 'Loading...' : `${rows.length} records — click any row to edit`}</div>
          </div>
        ...
        <div style={{ overflowY:'auto', flex:1 }}>
          {loading ? <Loader /> : rows.length === 0 ? (
            <div style={{ textAlign:'center', padding:60, color:'rgba(255,255,255,0.38)' }}>No records found</div>
          ) : (
              ...
              <thead>
                <tr style={{ background:'rgba(255,255,255,0.04)', position:'sticky', top:0 }}>
                  {['Date','Customer','Company','Disposition','Parts','Notes',''].map(h => (
                    <th key={h} style={{ textAlign:'left', padding:'10px 14px', fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.06em', whiteSpace:'nowrap', borderBottom:'2px solid rgba(255,255,255,0.09)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', background:i%2===0?'var(--card)':'var(--row-alt)', transition:'background 0.1s' }}
                    ...
                    <td style={{ padding:'10px 14px', color:'rgba(255,255,255,0.50)', fontFamily:'monospace', fontSize:12, whiteSpace:'nowrap' }}>{formatDateShort(r.created_at)}</td>
                    <td style={{ padding:'10px 14px', fontWeight:600, color:'var(--card)', whiteSpace:'nowrap' }}>{r.customer_name}</td>
                    <td style={{ padding:'10px 14px', color:'rgba(255,255,255,0.50)', fontSize:12 }}>{r.customer_company||'—'}</td>
                    <td style={{ padding:'10px 14px' }}><DispositionBadge disposition={r.disposition} /></td>
                    <td style={{ padding:'10px 14px', fontFamily:'monospace', fontSize:11, color:'rgba(255,255,255,0.65)', maxWidth:140, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r.requirements?.map(req => req.part_number).join(', ')||'—'}</td>
                    <td style={{ padding:'10px 14px', color:'rgba(255,255,255,0.38)', fontSize:12, maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{r.notes||'—'}</td>
...
function AELeadsTab({ dateFilters, onDrilldown }) {
  ...
  return (
    <div>
      <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:10 }}>Today</div>
      ...
      <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:10 }}>Period</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:20, marginBottom:20 }}>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          <STitle>Lead Trend — Won vs Total</STitle>
          {trendData.length === 0 ? <div style={{ height:160, display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(255,255,255,0.38)' }}>No data</div> : (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {(data.bySource||[]).filter(s=>s.source).slice(0,7).map((s,i) => (
              <div key={s.source} onClick={() => drill(`Source: ${s.source}`)} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:2 }}><span style={{ color:'rgba(255,255,255,0.65)' }}>{s.source}</span><span style={{ fontWeight:700 }}>{s.count}</span></div>
                <div style={{ height:4, background:'rgba(255,255,255,0.04)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:C[i%C.length], width:`${p.total>0?Math.round(s.count/p.total*100):0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
          ...
          {(data.byDisposition||[]).filter(d=>d.disposition).map((d,i) => {
            ...
            return (
              <div key={d.disposition} onClick={() => drill(d.disposition, { disposition:d.disposition })}
                style={{ background:'rgba(255,255,255,0.04)', borderRadius:12, padding:'12px 14px', border:'1px solid rgba(255,255,255,0.07)', cursor:'pointer', transition:'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor=C[i%C.length]; e.currentTarget.style.background='var(--card-2)' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor='var(--card-2)'; e.currentTarget.style.background='var(--card-2)' }}>
                <div style={{ fontSize:11, color:'rgba(255,255,255,0.50)', marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.disposition}</div>
                <div style={{ fontSize:20, fontWeight:800, color:'var(--card)', fontFamily:'"Bricolage Grotesque",sans-serif' }}>{d.count}</div>
                <div style={{ height:3, background:'var(--border-2)', borderRadius:4, marginTop:6 }}><div style={{ height:'100%', borderRadius:4, background:C[i%C.length], width:`${pct}%` }} /></div>
                <div style={{ fontSize:10, color:'rgba(255,255,255,0.38)', marginTop:3 }}>{pct}%</div>
              </div>
            )
          })}
        </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {(data.byDisposition||[]).slice(0,10).map((d,i) => (
              <div key={d.disposition} onClick={() => drill(d.disposition, { disposition:d.disposition })} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:2 }}><span style={{ color:'rgba(255,255,255,0.65)' }}>{d.disposition||'Unknown'}</span><span style={{ fontWeight:700 }}>{d.count}</span></div>
                <div style={{ height:4, background:'rgba(255,255,255,0.04)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:C[i%C.length], width:`${p.total>0?Math.round(d.count/p.total*100):0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={(data.trend||[]).map(t => ({ ...t, date:t.date?.slice(5) }))}>
              ...
              <YAxis tick={{ fontSize:10, fill:'var(--chart-axis)' }} axisLine={false} tickLine={false} allowDecimals={false} />
              <Tooltip content={<Tip />} />
        ...
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {(data.bySource||[]).filter(s=>s.source).map((s,i) => (
              <div key={s.source} onClick={() => drill(`Source: ${s.source}`)} style={{ cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:3 }}><span style={{ color:'rgba(255,255,255,0.65)' }}>{s.source}</span><span style={{ fontWeight:700 }}>{s.count}</span></div>
                <div style={{ height:5, background:'rgba(255,255,255,0.04)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:C[i], width:`${p.total>0?Math.round(s.count/p.total*100):0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
            {[['This Month',data.month.won,data.month.total,data.month.win_rate,'#10b981'],['This Year',data.year.won,data.year.total,data.year.win_rate,BRAND],['All Time',data.all.won,data.all.total,data.all.win_rate,'#6366f1']].map(([lbl,won,total,rate,clr]) => (
              <div key={lbl} style={{ background:'rgba(255,255,255,0.04)', borderRadius:12, padding:'12px 14px', textAlign:'center' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>{lbl}</div>
                <div style={{ fontSize:20, fontWeight:800, color:clr, fontFamily:'"Bricolage Grotesque",sans-serif' }}>{won}</div>
                <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)', marginTop:2 }}>won of {total}</div>
                <div style={{ marginTop:8, height:4, background:'var(--border-2)', borderRadius:4 }}><div style={{ height:'100%', borderRadius:4, background:clr, width:`${rate}%` }} /></div>
          ...
          {data.weeklyTrend?.length > 0 && (
            <ResponsiveContainer width="100%" height={100}>
              <LineChart data={data.weeklyTrend.map((w,i) => ({ ...w, week:`W${i+1}` }))}>
                <XAxis dataKey="week" tick={{ fontSize:10, fill:'var(--chart-axis)' }} axisLine={false} tickLine={false} />
                <YAxis hide />
                <Tooltip content={<Tip />} />
        ...
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          ...
          {!data.pipeline?.length ? <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', padding:'32px 0' }}>No active pipeline</div> : (
            <ResponsiveContainer width="100%" height={200}><BarChart data={data.pipeline.slice(0,7)} layout="vertical" barSize={12}><XAxis type="number" tick={{ fontSize:10, fill:'var(--chart-axis)' }} axisLine={false} tickLine={false} allowDecimals={false} /><YAxis type="category" dataKey="disposition" tick={{ fontSize:10, fill:'var(--text-2)' }} axisLine={false} tickLine={false} width={100} /><Tooltip content={<Tip />} /><Bar dataKey="count" name="Count" fill={BRAND} radius={[0,4,4,0]} /></BarChart></ResponsiveContainer>
          )}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          <STitle>📅 My Follow-ups <span style={{ fontSize:12, color:'rgba(255,255,255,0.38)', fontWeight:400 }}>{data.followups?.overdue?.length + data.followups?.today?.length > 0 ? `⚠ ${data.followups.overdue.length + data.followups.today.length} urgent` : ''}</span></STitle>
          {[...( data.followups?.overdue||[]).map(f=>({...f,urgency:'overdue'})), ...(data.followups?.today||[]).map(f=>({...f,urgency:'today'})), ...(data.followups?.upcoming||[]).map(f=>({...f,urgency:'upcoming'}))].slice(0,6).map(fu => (
            <div key={fu.id} onClick={() => onNavigate('inquiry-detail',{id:fu.inquiry_id})} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.04)', cursor:'pointer' }}>
              ...
              <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:600, color:'var(--card)' }}>{fu.customer_name}</div><div style={{ fontSize:11, color:'rgba(255,255,255,0.50)' }}>{fu.note}</div></div>
          ...
          {!data.followups?.overdue?.length && !data.followups?.today?.length && !data.followups?.upcoming?.length && <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', padding:'20px 0', fontSize:13 }}>✅ All caught up!</div>}
        </div>

        <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:18 }}>
          <STitle>🔔 Needs Attention <span style={{ fontSize:12, color:'rgba(255,255,255,0.38)', fontWeight:400 }}>No activity 7+ days</span></STitle>
          {!data.untouched?.length ? <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', padding:'20px 0', fontSize:13 }}>✅ All up to date!</div> : data.untouched.map(inq => (
            <div key={inq.id} onClick={() => onDrilldown({ title:'Needs Attention', type:inq.type, filters:{ disposition: inq.disposition } })}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontWeight:600, fontSize:13, color:'var(--card)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{inq.customer_name}</div>
                <DispositionBadge disposition={inq.disposition} />
              </div>
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)', flexShrink:0 }}>{inq.last_activity ? timeAgo(inq.last_activity) : timeAgo(inq.created_at)}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ padding:32, maxWidth:1200, fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
        <div>
          <h1 style={{ fontFamily:'"Bricolage Grotesque",sans-serif', fontWeight:800, fontSize:26, color:'var(--card)', margin:0 }}>{greeting()}, {user.name} 👋</h1>
          <p style={{ color:'rgba(255,255,255,0.38)', fontSize:14, marginTop:4 }}>{new Date().toLocaleDateString('en-US', { weekday:'long', month:'long', day:'numeric' })}</p>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          {[['◎ New Lead','lead','#3b82f6'],['↻ New Repeat','repeat','#6366f1'],['◈ New Order','online_order','#f59e0b']].map(([label,type,color]) => (
            <button key={type} onClick={() => setNewModal(type)}
              ...
              onMouseEnter={e => { e.currentTarget.style.background=color; e.currentTarget.style.color='var(--card)' }}
      ...
      <div style={{ display:'flex', gap:2, background:'rgba(255,255,255,0.04)', borderRadius:14, padding:4, marginBottom:24, width:'fit-content' }}>
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            style={{ padding:'9px 18px', borderRadius:10, border:'none', background:activeTab===tab.key?'var(--card)':'transparent', color:activeTab===tab.key?'#0f172a':'#64748b', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'"Plus Jakarta Sans",sans-serif', boxShadow:activeTab===tab.key?'0 1px 4px rgba(0,0,0,0.08)':'none', transition:'all 0.15s', whiteSpace:'nowrap' }}>
            {tab.label}
          </button>
        ))}
      </div>
