...
export default function CustomerDetail({ id }) {
  ...
  const handleDelete = async () => {
    if (!confirm(`Delete customer "${customer.name}" and all their inquiries? This cannot be undone.`)) return
    setDeleting(true)
    try { await api.deleteCustomer(id); navigate('customers') }
      ...
      <div className="card p-6 mb-5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              {editMode
                ? <input className="input text-xl font-bold mb-1" value={editForm.name} onChange={e => setEF('name',e.target.value)} />
