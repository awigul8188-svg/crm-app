...
function PartDetailModal({ assignmentId, onClose, onSaved }) {
  ...
  if (!part) return createPortal(
    <div style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'var(--card)', borderRadius:20, padding:40, color:'rgba(255,255,255,0.38)' }}>Loading...</div>
    </div>, document.body
  )
  ...
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:99999, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'var(--card)', borderRadius:20, boxShadow:'0 24px 80px rgba(0,0,0,0.25)', width:'100%', maxWidth:640, maxHeight:'92vh', display:'flex', flexDirection:'column', fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
        ...
        <div style={{ padding:'16px 22px 12px', borderBottom:'1px solid rgba(255,255,255,0.06)', flexShrink:0, background: part.is_delayed?'#fff5f5':part.not_in_stock?'var(--card-2)':'var(--card)', borderRadius:'20px 20px 0 0' }}>
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
            <div>
              ...
              <div style={{ fontSize:12, color:'rgba(255,255,255,0.50)' }}>Qty: {part.quantity||'—'} · {part.customer_name}{part.customer_company?` · ${part.customer_company}`:''}</div>
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)', marginTop:2 }}>AE: {part.ae_name||'—'} · Assigned {timeAgo(part.assigned_at)}</div>
        ...
        {/* Tabs */}
        <div style={{ display:'flex', gap:1, borderBottom:'1px solid rgba(255,255,255,0.06)', flexShrink:0 }}>
        ...
        <div style={{ overflowY:'auto', flex:1, padding:'18px 22px' }}>
          ...
          {tab==='quote' && (
            <div>
              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>Price *</div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
                <div><div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>Lead Time</div><SInput value={leadTime} onChange={e=>setLeadTime(e.target.value)} placeholder="e.g. 3-5 days" /></div>
                <div><div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>Supplier</div><SInput value={supplier} onChange={e=>setSupplier(e.target.value)} placeholder="e.g. ABC Electronics" /></div>
              </div>
              <div style={{ marginBottom:16 }}><div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>Quote Notes</div><STextarea value={quoteNotes} onChange={e=>setQuoteNotes(e.target.value)} placeholder="Additional notes about this quote..." /></div>
          ...
          {tab==='notes' && (
            <div>
              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.50)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>My Notes / Update</div>
          ...
          {tab==='comments' && (
            <div>
              ...
              {!part.comments?.length ? <div style={{ textAlign:'center', color:'rgba(255,255,255,0.38)', padding:32, fontSize:13 }}>No messages yet. Start a conversation with the AE.</div> : (
          ...
          {tab==='followups' && (
            <div>
              <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:12, padding:14, border:'1px solid rgba(255,255,255,0.09)', marginBottom:16 }}>
    ...
    <div onClick={onClick} style={{ background:part.is_delayed?'#fff5f5':part.not_in_stock?'var(--card-2)':'var(--card)', borderRadius:14, border:`1px solid ${part.is_delayed?'#fecaca':part.not_in_stock?'#e2e8f0':part.urgency==='critical'?'#fecaca':part.urgency==='high'?'#fed7aa':'var(--card-2)'}`, padding:'14px 18px', cursor:'pointer', transition:'all 0.15s', display:'flex', alignItems:'flex-start', gap:14, flexWrap:'wrap' }}
      ...
      {part.inquiry_type==='online_order' && part.selling_price && (
        <div style={{ textAlign:'center', flexShrink:0 }}>
          <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase' }}>Selling</div>
      ...
      {part.quote_id ? (
        <div style={{ background:'#f0fdf4', borderRadius:10, padding:'8px 12px', border:'1px solid #bbf7d0', flexShrink:0, textAlign:'center' }}>
          ...
          <div style={{ fontSize:10, color:'rgba(255,255,255,0.50)' }}>{part.condition}</div>
        </div>
      ) : (
  ...
  return (
    <div style={{ padding:28, maxWidth:1200, fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16, flexWrap:'wrap' }}>
        ...
        {preset==='custom' && <>
          <input type="date" value={customFrom} onChange={e=>setCustomFrom(e.target.value)} style={{ padding:'7px 10px', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, fontSize:12, outline:'none' }} />
          <span style={{ color:'rgba(255,255,255,0.38)' }}>→</span>
          <input type="date" value={customTo} onChange={e=>setCustomTo(e.target.value)} style={{ padding:'7px 10px', border:'1px solid rgba(255,255,255,0.09)', borderRadius:8, fontSize:12, outline:'none' }} />
        </>}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
            <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:20 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                ...
                <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:10, padding:'12px 14px', textAlign:'center' }}>
                  <div style={{ fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.38)', textTransform:'uppercase', marginBottom:4 }}>Completion Rate</div>
      ...
      {['lead','repeat','online_order'].includes(activeTab) && (
        <div>
          {partsLoading ? <div style={{ textAlign:'center', padding:48, color:'rgba(255,255,255,0.38)' }}>Loading...</div>
            : !partsResult?.parts?.length ? <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:48, textAlign:'center', color:'rgba(255,255,255,0.38)' }}><div style={{ fontSize:32, marginBottom:8 }}>{T[activeTab]?.icon}</div>No parts assigned</div>
            : (
              <div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)', marginBottom:12 }}>{partsResult.total} parts · 30 per page · sorted by urgency</div>
      ...
      {activeTab==='followups' && (
        <div>
          ...
          {!stats?.followups?.overdue?.length && !stats?.followups?.today?.length && !stats?.followups?.upcoming?.length && (
            <div style={{ background:'var(--card)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', padding:60, textAlign:'center' }}>
              <div style={{ fontSize:40, marginBottom:12 }}>✅</div>
              <div style={{ color:'rgba(255,255,255,0.38)', fontSize:16, fontWeight:600 }}>All caught up!</div>
            </div>
          )}
        </div>
      )}
