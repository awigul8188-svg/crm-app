  ...
  return (
    <div style={{ padding:'24px 28px',flex:1,overflowY:'auto',maxWidth:1300,fontFamily:'"Plus Jakarta Sans",sans-serif' }}>
      ...
      <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:22,flexWrap:'wrap',gap:12 }}>
        ...
        <button onClick={()=>navigate('purchasing')} style={{ padding:'9px 18px',borderRadius:10,border:'none',background:BRAND,color:'#060610',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'"Plus Jakarta Sans",sans-serif' }}>My Parts →</button>
      </div>

      {/* Hero status cards */}
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20 }}>
        {[
          {label:'Assigned to Me',value:s.assigned,color:BRAND,dark:true,sub:'total parts'},
          {label:'Pending Quote',value:s.pending,color:'#f59e0b',sub:`${s.overdue} overdue`},
          {label:'Quoted',value:s.quotedAll,color:'#10b981',sub:'completed'},
          {label:'Not In Stock',value:s.notInStock,color:'#6366f1',sub:'flagged'},
        ].map(card=>(
          <div key={card.label} onClick={()=>navigate('purchasing')} style={{ background:card.dark?'linear-gradient(135deg,#0d1117,#0a2020)':'var(--card)', borderRadius:16, border:card.dark?'none':'1px solid var(--border)', padding:'18px 20px', cursor:'pointer', transition:'all 0.15s', position:'relative', overflow:'hidden' }}
            onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow=`0 8px 20px ${card.color}20`}}
            onMouseLeave={e=>{e.currentTarget.style.transform='none';e.currentTarget.style.boxShadow='none'}}>
            {card.dark&&<div style={{ position:'absolute',top:-30,right:-30,width:100,height:100,borderRadius:'50%',background:`radial-gradient(circle,${card.color}30 0%,transparent 70%)`,pointerEvents:'none' }} />}
            <div style={{ fontSize:9,fontWeight:700,color:card.dark?'rgba(255,255,255,0.35)':'var(--text-3)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:8 }}>{card.label}</div>
            <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif',fontWeight:900,fontSize:36,color:card.dark?'#fff':card.color,lineHeight:1,marginBottom:4 }}>{card.value}</div>
            <div style={{ fontSize:12,color:card.dark?'rgba(255,255,255,0.4)':'var(--text-3)' }}>{card.sub}</div>
          </div>
        ))}
      </div>

      {/* Today's stats strip */}
      <div style={{ background:'linear-gradient(135deg,#0d1117,#0a2020)',borderRadius:16,padding:'18px 24px',marginBottom:20,display:'flex',gap:32,flexWrap:'wrap' }}>
        {[
          {label:'Quotes Today',value:s.todayQuotes,color:BRAND},
          {label:'This Week',value:s.weekQuotes,color:'#10b981'},
          {label:'Avg Response',value:s.avgHours?`${s.avgHours}h`:'—',color:'#6366f1'},
          {label:'Critical Pending',value:s.critical,color:s.critical>0?'#ef4444':'#94a3b8'},
          {label:'Overdue (4+ days)',value:s.overdue,color:s.overdue>0?'#f97316':'#94a3b8'},
        ].map((s,i)=>(
          <div key={s.label} style={{ display:'flex',flexDirection:'column',gap:4 }}>
            {i>0&&<div style={{ position:'absolute' }} />}
            <div style={{ fontSize:9,fontWeight:700,color:'rgba(255,255,255,0.35)',textTransform:'uppercase',letterSpacing:'0.1em' }}>{s.label}</div>
            <div style={{ fontFamily:'"Bricolage Grotesque",sans-serif',fontWeight:900,fontSize:24,color:s.color,lineHeight:1 }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'2fr 1fr',gap:16,marginBottom:16 }}>
        ...
        <div style={{ background:'var(--card)',borderRadius:16,border:'1px solid var(--border)',padding:'20px 22px' }}>
          ...
          <div style={{ display:'flex',gap:20,marginTop:14,paddingTop:14,borderTop:'1px solid var(--border)' }}>
            {[
              {label:'Best Day',value:`${Math.max(...dailyTrend.map(d=>d.count))} quotes`},
              {label:'Avg/Day',value:`${(dailyTrend.reduce((s,d)=>s+d.count,0)/dailyTrend.length||0).toFixed(1)} quotes`},
              {label:'Total 14d',value:`${dailyTrend.reduce((s,d)=>s+d.count,0)} quotes`},
            ].map(s=>(
              <div key={s.label}>
        ...
        <div style={{ background:'var(--card)',borderRadius:16,border:'1px solid var(--border)',padding:'20px 22px' }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:16,marginBottom:16 }}>
            <Donut segments={pendingDonutSegs} total={s.pending} label="PENDING" />
            <div style={{ flex:1,display:'flex',flexDirection:'column',gap:8 }}>
              {byUrgency.map(u=>(
                <div key={u.urgency} onClick={()=>navigate('purchasing')} style={{ display:'flex',alignItems:'center',gap:8,cursor:'pointer',padding:'4px 8px',borderRadius:8,transition:'background 0.1s' }}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--card-2)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                  <div style={{ width:10,height:10,borderRadius:3,background:URGENCY_COLOR[u.urgency]||'#94a3b8',flexShrink:0 }} />
              ...
              {byUrgency.length===0&&<div style={{ color:'var(--text-3)',fontSize:13,textAlign:'center',padding:'20px 0' }}>All clear 🎉</div>}
            </div>
          </div>
          {/* By type */}
          <div style={{ paddingTop:14,borderTop:'1px solid var(--border)' }}>
            <div style={{ fontSize:11,fontWeight:700,color:'var(--text-3)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10 }}>By Inquiry Type</div>
            {byType.map(t=>(
              <div key={t.type} style={{ display:'flex',alignItems:'center',gap:8,marginBottom:8 }}>
                ...
                <span style={{ fontSize:12,color:BRAND,fontWeight:700 }}>{t.quoted}</span>
                <span style={{ fontSize:11,color:'var(--text-3)' }}>/{t.total}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:16 }}>
        ...
        <div style={{ background:'var(--card)',borderRadius:16,border:'1px solid var(--border)',overflow:'hidden' }}>
          ...
          <div style={{ padding:'12px 16px',maxHeight:300,overflowY:'auto' }}>
            {[...followups.overdue.map(f=>({...f,urgency:'overdue'})),...followups.today.map(f=>({...f,urgency:'today'})),...followups.upcoming].length===0 ? (
              <div style={{ textAlign:'center',padding:'32px 0',color:'var(--text-3)',fontSize:13 }}>✅ No pending follow-ups</div>
            ) : (
              <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                {[...followups.overdue.map(f=>({...f,urgency:'overdue'})),...followups.today.map(f=>({...f,urgency:'today'})),...followups.upcoming.slice(0,4).map(f=>({...f,urgency:'upcoming'}))].map(fu=>(
                  <div key={fu.id} style={{ display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,background:'var(--card-2)',border:`1px solid ${fu.urgency==='overdue'?'#fecaca':fu.urgency==='today'?'#fde68a':'var(--border)'}` }}>
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontWeight:600,fontSize:12,color:'var(--text)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{fu.part_number} — {fu.customer_name}</div>
                      <div style={{ fontSize:11,color:'var(--text-3)',marginTop:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{fu.note}</div>
                    </div>
                    <div style={{ fontSize:11,fontWeight:700,color:fu.urgency==='overdue'?'#ef4444':fu.urgency==='today'?'#f59e0b':'var(--text-3)',flexShrink:0 }}>{formatDate(fu.follow_up_date)}</div>
                    <button onClick={()=>completeFollowup(fu.id)} style={{ width:26,height:26,borderRadius:8,border:'1px solid var(--border)',background:'var(--card)',cursor:'pointer',fontSize:13,color:'var(--text-3)',flexShrink:0 }}>✓</button>
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{ background:'var(--card)',borderRadius:16,border:'1px solid var(--border)',overflow:'hidden' }}>
          ...
          <div style={{ padding:'12px 16px',maxHeight:300,overflowY:'auto' }}>
            {!recentQuotes?.length ? (
              <div style={{ textAlign:'center',padding:'32px 0',color:'var(--text-3)',fontSize:13 }}>No quotes submitted yet</div>
            ) : (
              <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                {recentQuotes.map((q,i)=>(
                  <div key={q.id} style={{ display:'flex',alignItems:'center',gap:12,padding:'10px 12px',borderRadius:10,background:'var(--card-2)',border:'1px solid var(--border)' }}>
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ fontSize:11,color:'var(--text-3)',marginTop:1 }}>{q.customer_name}</div>
                    </div>
      ...
      <div style={{ background:'var(--card)',borderRadius:16,border:`1px solid ${BRAND}30`,padding:'18px 22px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:12 }}>
          {[
            s.overdue>0 && { icon:'⚠️', text:`${s.overdue} part${s.overdue>1?'s are':' is'} overdue (4+ days) — quote these first`, color:'#ef4444' },
            s.critical>0 && { icon:'🚨', text:`${s.critical} critical urgency part${s.critical>1?'s':''}— customer-flagged`, color:'#ef4444' },
            s.avgHours && s.avgHours > 24 && { icon:'⏱', text:`Avg response time ${s.avgHours}h — aim for under 24h for better performance`, color:'#f59e0b' },
            s.todayQuotes===0 && { icon:'📋', text:`No quotes submitted today — ${s.pending} parts waiting for quotes`, color:'#f59e0b' },
            s.todayQuotes>0 && { icon:'🎯', text:`${s.todayQuotes} quotes submitted today — great work!`, color:'#10b981' },
            s.pending===0 && { icon:'🏆', text:'All caught up! No pending parts in your queue.', color:BRAND },
