...
// GET /api/admin/schema — returns full DB schema
...
router.post('/query', (req, res) => {
  ...
  try {
    ...
    const rows = db.prepare(sql).all(...actualParams);
...
router.get('/stats', (req, res) => {
  ...
  try {
    const stats = {
      ...
      by_person:         db.prepare("SELECT u.name, COUNT(*) as count FROM inquiries i JOIN users u ON i.assigned_to=u.id GROUP BY i.assigned_to, u.name ORDER BY count DESC").all(),
